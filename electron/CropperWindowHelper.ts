...
const CROPPER_CONFIG = {
  ...
  MIN_SELECTION_SIZE: parseInt(
    process.env.CROPPER_MIN_SELECTION_SIZE || "5",
    10,
  ),
...
export class CropperWindowHelper {
  ...
  private resolvePromise: ((value: Electron.Rectangle | null) => void) | null =
    null;
  ...
  private readonly confirmedListener: (
    event: IpcMainEvent,
    bounds: unknown,
  ) => void;
  ...
  constructor() {
    ...
    this.confirmedListener = (event, bounds: unknown) => {
      ...
      if (!isRectangle(bounds)) {
        console.error(
          "[CropperWindowHelper] Invalid bounds type received:",
          typeof bounds,
        );
      ...
      if (!cropperBounds) {
        console.error(
          "[CropperWindowHelper] cropper window missing on confirmed — refusing selection",
        );
        this.rejectCurrentSelection(null);
        this.hideOrClose();
        return;
      ...
      if (!this.validateBounds(globalBounds)) {
        console.error(
          "[CropperWindowHelper] Invalid bounds received:",
          globalBounds,
        );
        this.rejectCurrentSelection(null);
        this.hideOrClose();
    ...
    this.beforeQuitHandler = () => {
      if (!this.isDisposed) {
        console.log(
          "[CropperWindowHelper] before-quit: auto-disposing IPC listeners",
        );
  ...
  private validateBounds(bounds: Electron.Rectangle): boolean {
    ...
    if (
      ...
      console.warn(
        "[CropperWindowHelper] Invalid bounds: contains NaN or Infinity",
      );
      return false;
    }
    ...
    if (width <= 0 || height <= 0) {
      console.warn(
        "[CropperWindowHelper] Invalid bounds: zero or negative dimensions",
        { width, height },
      );
      return false;
    }
    ...
    if (
      width < CROPPER_CONFIG.MIN_SELECTION_SIZE ||
      height < CROPPER_CONFIG.MIN_SELECTION_SIZE
    ) {
      console.warn("[CropperWindowHelper] Selection too small", {
        width,
        height,
        minSize: CROPPER_CONFIG.MIN_SELECTION_SIZE,
      });
      return false;
    }
    ...
    if (
      x < combinedBounds.x ||
      y < combinedBounds.y ||
      selectionRight > combinedRight ||
      selectionBottom > combinedBottom
    ) {
      console.warn(
        "[CropperWindowHelper] Bounds exceed combined multi-monitor viewport",
        {
    ...
    console.log(
      `[CropperWindowHelper] validateBounds PASSED: x=${x}, y=${y}, w=${width}, h=${height}`,
    );
    return true;
  }
  ...
  private resolveCurrentSelection(bounds: Electron.Rectangle | null): void {
    if (!this.isWaitingForSelection) {
      console.warn(
        "[CropperWindowHelper] resolveCurrentSelection called but not waiting for selection",
      );
      return;
    }
  ...
  private rejectCurrentSelection(reason?: unknown): void {
    if (!this.isWaitingForSelection) {
      console.warn(
        "[CropperWindowHelper] rejectCurrentSelection called but not waiting for selection",
      );
      return;
    }
    if (this.resolvePromise) {
  ...
  public preload(): void {
    if (this.isDisposed) {
      console.warn(
        "[CropperWindowHelper] Cannot preload: instance has been disposed",
      );
      return;
    }
    if (!this.cropperWindow || this.cropperWindow.isDestroyed()) {
   ...
   */
  public async showCropper(
    timeout = 30000,
  ): Promise<Electron.Rectangle | null> {
    if (this.isDisposed) {
      console.warn(
        "[CropperWindowHelper] Cannot show cropper: instance has been disposed",
      );
      return null;
    }
    ...
    return new Promise((resolve, reject) => {
      ...
      if (this.cropperWindow && !this.cropperWindow.isDestroyed()) {
        ...
        const hudPosition = targetDisplay
          ? {
              x:
                ...
                Math.round(targetDisplay.bounds.width / 2),
        ...
        console.log(
          `[CropperWindowHelper] Cursor at ${JSON.stringify(cursorPosition)}, display bounds: ${targetDisplay ? JSON.stringify(targetDisplay.bounds) : "unknown"}`,
        );
        console.log(
          `[CropperWindowHelper] HUD position: ${JSON.stringify(hudPosition)}`,
        );
  ...
  private applyOpacityShield(): void {
    ...
    if (process.platform === "win32") {
      ...
      this.opacityTimeout = setTimeout(() => {
        if (
          this.cropperWindow &&
          !this.cropperWindow.isDestroyed() &&
          !this.isDisposed
        ) {
  ...
  private createWindow(showImmediately: boolean): void {
    if (this.isDisposed) {
      console.warn(
        "[CropperWindowHelper] Cannot create window: instance has been disposed",
      );
      return;
    }
    ...
    console.log(
      `[CropperWindowHelper] Creating cropper window with multi-monitor bounds:`,
      combinedBounds,
    );
    ...
    console.log(
      `[CropperWindowHelper] Window created. Actual bounds:`,
      actualBounds,
    );
    console.log(
      `[CropperWindowHelper] Expected bounds: {x:${combinedBounds.x}, y:${combinedBounds.y}, width:${combinedBounds.width}, height:${combinedBounds.height}}`,
    );

    if (process.platform === "darwin") {
      this.cropperWindow.setVisibleOnAllWorkspaces(true, {
        ...
        try {
          ...
          if (native && typeof native.applyStealthToWindow === "function") {
            native.applyStealthToWindow(
              this.cropperWindow.getNativeWindowHandle(),
            );
          }
        } catch (e) {
          console.error(
            "[CropperWindowHelper] applyStealthToWindow failed:",
            e,
          );
        }
      }
    ...
    this.cropperWindow.on("show", () => {
      ...
      try {
        ...
        console.error(
          "[CropperWindowHelper] failed to stop stealth tap on show:",
          e,
        );
      }
    });
  ...
  private async loadCropperUrlWithRetry(): Promise<void> {
    ...
    for (
      let attempt = 1;
      attempt <= CROPPER_CONFIG.MAX_LOAD_RETRIES;
      attempt++
    ) {
      try {
        await this.cropperWindow!.loadURL(cropperUrl);
        console.log(
          `[CropperWindowHelper] URL loaded successfully (attempt ${attempt})`,
        );
        return;
      } catch (error) {
        console.error(
          `[CropperWindowHelper] Failed to load URL (attempt ${attempt}/${CROPPER_CONFIG.MAX_LOAD_RETRIES}):`,
          error,
        );
        ...
        if (attempt === CROPPER_CONFIG.MAX_LOAD_RETRIES) {
          ...
          this.rejectCurrentSelection(
            new Error("Failed to load cropper UI after multiple attempts"),
          );
          this.hideOrClose();
          throw error;
        }
  ...
  public dispose(): void {
    if (this.isDisposed) {
      console.warn(
        "[CropperWindowHelper] dispose() called but already disposed",
      );
      return;
    }
    ...
    if (this.resolvePromise) {
      ...
      this.isWaitingForSelection = false;
      console.log(
        "[CropperWindowHelper] Pending selection rejected due to disposal",
      );
    }
